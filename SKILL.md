...
- `modified_files = []` — file paths modified in current round's Phase E; reset each round
...
Role count baseline (adjust by complexity and risk): empty/trivial file (0 lines or <10 non-blank lines) → warn user, minimum 1 reviewer; 1-199 lines: 3-5; 200-499 lines: 5-8; 500-2000 lines: 8-12; 2000+ lines or multi-file monorepo: 15-20. Max 20.

### 3. Generate Severity Calibration

Produce domain-specific severity calibration (e.g., style preference = SUGGESTION; injection vector = ERROR).
...
If user explicitly requests to skip roster review (e.g., "just start", "go"), proceed directly.
...
Pre-check:
- Success < 80% → automatically retry failed reviewers once (shares the 1-retry-per-reviewer limit), then continue with available results (mark PARTIAL_REVIEW)
...
2. Regression check (round 2+): any re-spawned reviewer's score drops ≥2 OR drops below PASS_THRESHOLD from above → REGRESSION, escalate (revert last round's Phase E edits if any + re-run / accept + continue / abort). Drop of 1 → MINOR_REGRESSION (logged, no pause).
   ...
   - Excluded: CARRY reviewers, re-spawned-from-CARRY reviewers, reviewers with no valid score in previous round (TIMEOUT/PARSE_ERROR — treat as first-time scoring)
3. All scores >= PASS_THRESHOLD → final report
...
Phase C failure (agent error, empty response, or invalid JSON after one retry, independent of Phase B retry quota) → carry Phase A score forward as revised_score, mark ABSENT. ABSENT reviewers are NOT exempt from the all-pass threshold check. If the same reviewer is ABSENT for 2 consecutive rounds, escalate to user.
...
Your most recent findings (full JSON — from last spawned round): {reviewer_own_previous_findings}
...
6. Failures + AUTO_FIX on/safe + no ERROR/WARNING findings (including zero findings) → carry passed reviewers, re-spawn only failing reviewers in next Phase A (note LOW_SCORE_NO_FIXABLE in round summary)
7. Failures + AUTO_FIX=off → carry passed reviewers, re-spawn only failing reviewers in next Phase A
...
- If fix_old is not found in the target file, mark FIX_FAILED. If fix_old matches multiple locations and the finding's location does not disambiguate, mark DEFERRED.
- Two findings conflict if they target the same file and their location line ranges overlap and both have fix_old + fix_new → mark CONFLICT, skip both
- Vague suggestion → mark DEFERRED
- Edit fails → mark FIX_FAILED
- Same finding FIX_FAILED 3 consecutive rounds → MANUAL_REQUIRED (never retried; still scored). Match by id (fallback: file+severity+slug when line numbers shift); uncertain match = new finding (counter resets). Secondary: if same file+line range with identical fix_old as a prior FIX_FAILED, inherit counter regardless of id.
- AUTO_FIX=safe → present all proposed fixes as a batch for user confirmation before applying (exception to auto-continue)
...
4. When in doubt, prefer re-spawning
...
```json
{
  ...
  "sub_agent_findings": "<array of objects with same schema; [] if none. Depth >= MAX_DEPTH MUST output []. Moderator flattens all nested findings into the reviewer's list for processing.>",
...
|----------|-------|-------|---------|------------|--------|
Status values: PASS / FAIL / CARRY / CARRY_UNRESOLVED / TIMEOUT / PARSE_ERROR / ABSENT / PARTIAL_REVIEW (round-level: REGRESSION / MINOR_REGRESSION / LOW_SCORE_NO_FIXABLE)
...
- Estimated token cost: 20 reviewers × 5 rounds ≈ 3-8M tokens
